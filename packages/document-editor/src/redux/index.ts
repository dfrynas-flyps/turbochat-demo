...
import {
  ...
  normalizeTemplateDataRead,
} from '../helpers/templateDataCleanup'
...
export const templateSlice = createSlice({
  ...
  reducers: {
    ...
    setActiveVar: (state: TemplateState, action: PayloadAction<TemplateState['activeVar']>) => {
    ...
    setActiveSection: (state, action: PayloadAction<{ id: string } | null>) => {
      state.activeSection = action.payload
    },
    setDocumentHeading: (state, action: PayloadAction<string>) => {
      if (state.data) {
    ...
    addNewVariable: (state, action: PayloadAction<VariableRegistryEntry>) => {
    ...
    updateVariable: (
      ...
      state.data.sections = state.data.sections.map((section) => {
        const shouldBeSectionEditorUpdated =
          (!isOnlySectionActive || section.id !== state.activeSection?.id) && section.name.includes(variablePattern)

        const updatedStatements = section.statements.map((statement) => {
          const shouldBeStatementEditorUpdated =
            (!isOnlyStatementActive || statement.id !== state.activeStatement?.id) &&
    ...
    toggleLastStatement: (state, action: PayloadAction<{ id: string; lastStatement: boolean }>) => {
      const {
        payload: { id, lastStatement },
      } = action
    ...
    typeoverStatement: (
      ...
      const {
        payload: { text, id, newVariables, typeoverMode },
      } = action
      updateStatementField({
        state,
        id: id,
    ...
    setActionAutoApplied: (state, action: PayloadAction<{ id: string; actionAutoApplied: ActionNames }>) => {
      updateStatementField({
        state,
        id: action.payload.id,
    ...
    addNewStatement: (state, action: PayloadAction<TemplateState['newStatement']>) => {
      if (!state.data) {
    ...
    addNewSection: (state, action: PayloadAction<{ name: string }>) => {
      if (!state.data) {
    ...
    setAlert: (state, action: PayloadAction<{ text: string; type?: AlertColor }>) => {
      state.alert = action.payload
    },
    setTrackLoadedTemplateAfterCreation: (state, action: PayloadAction<boolean>) => {
      state.trackLoadedTemplateAfterCreation = action.payload
    },
    setProjectData: (state, action: PayloadAction<TemplateState['projectData']>) => {
      state.projectData = action.payload
    },
    setNewStatement: (state, action: PayloadAction<TemplateState['newStatement']>) => {
      state.newStatement = action.payload
    },
    setImageAsConfirmed: (state, action: PayloadAction<{ id: string }>) => {
      updateStatementField({ state, id: action.payload.id, newValues: { isImageReady: true } })
      state.triggerDataSave = Date.now()
    },
    setRegeneratedSection: (state, action: PayloadAction<TemplateSection>) => {
    ...
    deleteStatement: (state, action: PayloadAction<{ id: string }>) => {
      if (!state.data) {
    ...
    applyDeleteHint: (state, action: PayloadAction<{ id: string }>) => {
      updateStatementField({
        state,
        id: action.payload.id,
    ...
    toggleSectionSoftDelete: (state, action: PayloadAction<{ id: string; isDeleted: boolean }>) => {
      if (!state.data) {
    ...
    renameSection: (state, action: PayloadAction<{ id: string; newName: string }>) => {
      const { id, newName } = action.payload
      updateSectionField({ state, id, newValues: { name: newName } })
      state.triggerDataSave = Date.now()
    },
    changeSectionHeadingSize: (state, action: PayloadAction<{ id: string; newName: string }>) => {
      const { id, newName } = action.payload
      updateSectionField({ state, id, newValues: { header_size: newName } })
      state.triggerDataSave = Date.now()
    },
    setCreateState: (state, action: PayloadAction<{ loading: boolean; error: boolean; message?: string }>) => {
      state.createState = action.payload
    },
    applyVariablesOffHint: (state, action: PayloadAction<{ id: string }>) => {
      updateStatementField({
        state,
        id: action.payload.id,
        newValues: {
    ...
    toggleStatementVariables: (state, action: PayloadAction<{ statementId: string; turnOff: boolean }>) => {
      updateStatementField({
        state,
    ...
    applyOptionalHint: (state, action: PayloadAction<{ id: string }>) => {
      updateStatementField({
        state,
        id: action.payload.id,
        newValues: {
    ...
    toggleStatementOptional: (state, action: PayloadAction<{ statementId: string; optional: boolean }>) => {
      updateStatementField({
        state,
        id: action.payload.statementId,
    ...
    toggleDeleteModal: (state, action: PayloadAction<{ id: string; type: DeleteModalTypes } | null>) => {
      state.deleteModal = action.payload
    },
    setPromptToDelete: (state, action: PayloadAction<{ id: string }>) => {
      state.promptToDelete = action.payload.id
    },
    setPromptId: (state, action: PayloadAction<{ id: string; type: PromptTypes } | null>) => {
    ...
    updateAndSaveSection: (state, action: PayloadAction<{ id: string; newValues: Partial<TemplateSection> }>) => {
      const { id, newValues } = action.payload
      updateSectionField({ state, id, newValues: newValues })
      state.triggerDataSave = Date.now()
    },
    resetState: () => initialState,
    updateDocumentHeader: (state, action: PayloadAction<Array<HeaderFieldState>>) => {
      if (state.data === null) return
      state.data.documentHeader = action.payload
      state.triggerDataSave = Date.now()
    },
    setIndentLevel: (state, action: PayloadAction<{ statementId: string; indentLevel: number }>) => {
      if (!state.data) {
  ...
  extraReducers: (builder) => {
    ...
    //   }
    // })
    builder.addCase(fetchData.fulfilled, (state, action: PayloadAction<TemplateData>) => {
    ...
    builder.addCase(fetchData.rejected, (state, { payload }) => {
      if (payload?.status === 404) state.alert = { text: 'This template no longer exists', type: 'error' }
      else state.alert = { text: 'Could not fetch template', type: 'error' }
    })
...
export const {
  ...
  setFetchError,
  setAlert,
  setTrackLoadedTemplateAfterCreation,
  setData,
  setNewStatement,
  addNewStatement,
  addNewSection,
  ...
  resetState,
  setCreateState,
  applyVariablesOffHint,
  toggleStatementVariables,
  applyOptionalHint,
  toggleStatementOptional,
  toggleDeleteModal,
  toggleLastStatement,
  typeoverStatement,
  setActionAutoApplied,
  applyDeleteHint,
