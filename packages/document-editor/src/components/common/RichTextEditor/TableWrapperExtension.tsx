...
const TableWrapperNode = (props: NodeViewRendererProps) => {
  const { editor } = props
  ...
  return (
    <NodeViewWrapper>
      <Container>
        <NodeViewContent />
        <AddButton addRow onClick={() => editor.commands.addRowAfter()} aria-label="Add row">
          <PlusIcon />
        </AddButton>
        <AddButton onClick={() => editor.commands.addColumnAfter()} aria-label="Add column">
          <PlusIcon />
        </AddButton>
      </Container>
    </NodeViewWrapper>
...
const AddButton = styled('button', { shouldForwardProp: (prop) => prop !== 'addRow' })<{
  addRow?: boolean
}>(({ theme, addRow }) => ({
  display: 'flex',
  alignItems: 'center',
