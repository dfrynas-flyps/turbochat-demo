...
import { ActiveVariable } from '../redux/types'
import {
  TemplateSection,
  TemplateStatement,
  VariableHistoryEntryAuthorType,
  VariableRegistryEntry,
} from '../types/editor'
import {
  DEFAULT_NEW_VARIABLE_VALUE,
  VARIABLE_NODE_SELECTOR,
  VAR_INPUT_EXTENSION_NAME,
  VAR_INPUT_TAG_NAME,
...
const getVariableIdsFromSelection = (
  range: Range,
  startVariableElement: Element | null,
  endVariableElement: Element | null
): string[] => {
  const variableIds: Array<string | null> = []
  ...
  return variableIds.filter((id): id is string => id !== null)
}

const findClosestVariableElement = (node: Node | Element): Element | null => {
...
const VariableToggleButton: React.FC<VariableToggleButtonProps> = ({
  editor,
  onVariableRemove,
  statementId,
  sectionId,
  ...
  const [mode, setMode] = useState<ACTION_MODES>(ACTION_MODES.INSERT)
  const [selectedVariables, setSelectedVariables] = useState<Array<ActiveVariable['variableDisplayId']>>([])
  ...
  const focusVariableByDisplayId = useCallback((variableDisplayId: string) => {
    const variableNode = document.querySelector<HTMLElement>(`[data-variable-display-id="${variableDisplayId}"]`)
    if (variableNode) {
      variableNode.focus()
    }
  }, [])
  ...
  const createVariable = useCallback(() => {
    ...
    const newVariable: VariableRegistryEntry = {
      initialValue: '',
      name: variableName,
      value: variableValue,
  ...
  const removeVariable = useCallback(() => {
    ...
    const newContent = replaceEditorNodes(editor.getJSON(), (node) => {
      if (
        node.type === VAR_INPUT_EXTENSION_NAME &&
  ...
  return (
    <>
      <Tooltip placement="top" title={<span>{buttonLabel}</span>}>
        <ToolbarButton aria-label={buttonLabel} onClick={onClick}>
          <BoltIcon color="#fff" />
