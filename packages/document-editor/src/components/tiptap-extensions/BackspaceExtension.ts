...
import { StatementTypes, TemplateSection, TemplateStatement, isListStatement } from '../../types/editor'

export const getEditorsCurrentNodeLength = (editor: Editor) => {
  const { state } = editor
...
const changeStatementTypeToText = (activeStatementId: string) => {
  reduxStore.dispatch(
    updateStatement({
...
const concatWithPreviousAndDeleteCurrentStatement = (
  activeStatementId: string,
  previousStatement: TemplateStatement,
  focusPositionFromContentEnd: number
) => {
...
const concatStatementWithSection = (
  activeStatementId: string,
