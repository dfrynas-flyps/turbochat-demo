...
const isDomRectEqual = (a: Partial<DOMRect>, b: Partial<DOMRect>) => {
  return (
    a?.bottom === b?.bottom &&
    a?.height === b?.height &&
    a?.left === b?.left &&
...
export const useToolbarPosition = ({ editor, toolbarWrapperRef }: ToolbarPositionProps): UseToolbarPositionReturn => {
  ...
  const handleStatementAndToolbarBlur = useCallback(
