...
const normalizeHint = <T = string>(hint: T): T => {
  if (typeof hint === 'string') {
    return hint.replace(/[\])}[{(]/g, '') as T
  }
  return hint
}
...
export const useStatementProposedAction = ({ statement }: UseStatementProposedActionArgs) => {
  ...
  const actions: Record<ActionNames, ActionType> = useMemo(
    () => ({
      ...
      [ActionNames.TYPEOVER]: {
        ...
        divider: true,
        text: 'Typeover',
        isVisible: (statement?: TemplateStatement) => {
