...
import { addNewStatement, 
// getTitle,
setActiveSection, setActiveStatement, setNewStatement, updateStatement, } from '../redux';
// import { DEFAULT_STATEMENT_TITLE, LoadState } from '../redux/helpers'
import { selectActiveSectionId, selectActiveStatement, selectNewStatement, selectProjectData, selectVariableRegistry, } from '../redux/selectors';
...
const RichTextEditor = () => {
    ...
    const [, setInitialContent] = useRefState(null);
    const statementEditorRef = useRef(null);
    ...
    const listStyleType = useMemo(() => {
        ...
        const previousStatement = section.statements[statementIndex - 1];
    ...
    const orderedListItemIndex = useMemo(() => {
        if (statement.type !== StatementTypes.ORDERED_LIST) {
            return null;
        }
    ...
    const debouncedStatementUpdate = useMemo(() => debounce((newText) => {
        dispatch(updateStatement({ id: statement.id, updatePayload: { text: newText } }));
    }, 500), [dispatch, statement.id]);
    ...
    useEffect(() => {
        if (!editor || activeStatement?.id !== statement.id)
            return;
    ...
    const onStatementClick = useCallback(() => {
        if (!isStatementActive && statement.typeoverMode && statementEditorRef.current) {
            ...
            const statementContent = statementEditorRef.current.childNodes[0];
    ...
    return (_jsxs(StyledTypeoverLabelContainer, { children: [statement.typeoverMode && (_jsxs(TypeoverLabel, { children: ["Please provide 1-2 sentence description", statement.name && ` of ${statement.name}`] })), _jsx(StyledEditor, { editor: editor, orderedListItemIndex: orderedListItemIndex, isList: isListStatement(statement), listStyleType: listStyleType, isTable: statement.type === 'table', isTypeover: Boolean(statement.typeoverMode), isOptional: Boolean(statement.optionalStatement), spellCheck: "false", ref: statementEditorRef, onClick: onStatementClick, ...(statement.typeoverMode && { 'data-tab-navigation': 'click' }), indentLevelInPx: indentFeature.indentLevelInPx }), _jsx(DynamicToolbar, { isOpen: isStatementActive, editor: editor, handleAddTable: handleAddTable, handleUnorderedListToggle: onUnorderedListToggle, handleOrderedListToggle: onOrderedListToggle, handleAddImage: handleAddImage, isOrderedListActive: statement.type === StatementTypes.ORDERED_LIST, isUnorderedListActive: statement.type === StatementTypes.UNORDERED_LIST, indentFeature: indentFeature, children: _jsxs(_Fragment, { children: [!statement.typeoverMode && editor && (_jsx(VariableToggleButton, { editor: editor, sectionId: section.id, statementId: statement.id, onVariableRemove: () => {
                                dispatch(updateStatement({ id: statement.id, updatePayload: { text: editor.getHTML() } }));
                            } })), editor?.can().deleteRow() && (_jsxs(_Fragment, { children: [_jsx(Divider, {}), _jsx(TableBtn, { onClick: () => {
                                        editor.commands.deleteRow();
                                    }, children: "Delete row" })] })), editor?.can().deleteColumn() && (_jsx(TableBtn, { onClick: () => {
...
const StyledEditor = styled(EditorContent, { shouldForwardProp })(({ theme, isTable, isList, listStyleType, orderedListItemIndex, isTypeover, isOptional, indentLevelInPx }) => ({
    width: 'calc(100% - 36px)',
    minHeight: '80px',
    overflowX: 'hidden',
    ...(isTable && {
