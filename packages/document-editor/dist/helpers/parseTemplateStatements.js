...
export const createHTMLTag = ({ tagName, innerText, attrs, }) => {
    const attributesString = Object.keys(attrs)
        .map((key) => `${key}="${attrs[key]}"`)
        .join(' ');
    return `<${tagName} ${attributesString}>${innerText}</${tagName}>`;
};
export const getVariableValue = (variable, projectData) => {
 ...
 */
export const formatStatementText = ({ statement, sectionId, variableRegistry, projectData, variableOutput = 'editable', }) => {
 ...
 */
export const formatSectionName = ({ section, variableRegistry, projectData, variableOutput = 'editable', }) => {
...
const transformVariables = (text) => {
    return sanitizeHtml(text, {
        ...
        transformTags: {
            [VAR_INPUT_TAG_NAME]: function (tagName, attribs) {
                return {
                    tagName: tagName,
                    text: `{${attribs['name']}}`,
                    attribs,
 ...
 */
export const editorHtmlToSectionName = (sectionName) => {
 ...
 * @returns
 */
export const editorHtmlToStatementText = (statementText) => {
...
//# sourceMappingURL=parseTemplateStatements.js.map