...
const listTasks = ({ session }: { session: Session }) =>
	tool({
		description:
			"List all tasks. Whenever user asks for tasks list you should pick this tool and fetch existing tasks in system.",
		parameters: z.object({
		...
		execute: async ({ status }) => {
			try {
				const response = await fetch(
					`${process.env.SERVER_URL}/tasks${status ? `?status=${status}` : ""}`,
				);
				if (!response.ok) {
