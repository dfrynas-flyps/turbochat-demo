...
const updateTask = ({ session }: { session: Session }) =>
	tool({
		description:
			"Whenever user asks for updating particular task, you should use this tool and send whole task including updated data",
		parameters: z.object({
		...
		execute: async ({ id, title, description, status, dueDate }) => {
			try {
				...
				if (!response.ok) {
					...
					throw new Error(
						`Failed to update task: ${JSON.stringify(errorData)}`,
					);
				}
